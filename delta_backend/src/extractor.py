...
def get_valid_address(patient, occurrence_time):
    ...
    selected_address = next((a for a in valid_addresses if a.get("use") == "home" and a.get("type") != "postal"),
                                    next((a for a in valid_addresses if a.get("use") != "old" and a.get("type") != "postal"),
...
def extract_site_code(json_data):
    ...
    selected_performer = next((p for p in valid_performers if p["actor"].get("type") == "Organization" and 
                                p["actor"].get("identifier", {}).get("system") == "https://fhir.nhs.uk/Id/ods-organization-code"),
                                valid_performers[0])
